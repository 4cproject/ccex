	...
	<tbody>
        <?php for($i=0, $n = count($this->costs);$i<$n;$i++) { ?>
			...
			<tr>
                ...
                <?php if(isset($this->editable) && $this->editable) { ?>
				    <td class="text-center tour-step tour-step-coll-cost-edit"><a class="text-primary" data-toggle="tooltip" data-placement="left" title="Click here to edit the cost unit of your cost data set" href="<?php echo JRoute::_('index.php?option=com_ccex&view=cost&layout=edit&cost_id=' . $cost->cost_id) ?>">edit</a></td>
			    <?php } ?>
            </tr>
        <?php } ?>
	</tbody>
	<tfoot>
		<tr style="color: #999">
			...
			<?php } else { ?>
                ...
                <?php } else { ?>
                <td class="text-left" colspan="7">To add costs, please <a class="text-primary" href="<?php echo JRoute::_('index.php?view=collection&layout=edit&collection_id=' . $this->interval->collection()->collection_id) ?>">edit cost data sets</a>.</td>
                <?php } ?>
			<?php } ?>
            <?php if(isset($this->editable) && $this->editable) { ?>
    ...
    <?php if(isset($this->editable) && $this->editable) { ?>
        <span class="tour-step tour-step-coll-add-cost-unit pull-right">
            <?php if(isset($this->interval->interval_id)){ ?>
                <a class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="left" title="Click here to add curation costs to this cost data set" href="javascript:void(0)" onclick="<?php echo 'ccexUpdate(\'collection\', \'' . JRoute::_('index.php?option=com_ccex&view=cost&layout=add&interval_id=' . $this->interval->interval_id ) . '\', true)'; ?>">Add new cost unit</a>
            <?php } else { ?>
                <a class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="left" title="Click here to add curation costs to this cost data set" href="javascript:void(0)" onclick="<?php echo 'ccexCreate(\'collection\', \'' . JRoute::_('index.php?option=com_ccex&view=cost&layout=add&interval_id=' ) . '\', true, \'interval\')'; ?>">Add new cost unit</a>
            <?php } ?>
        </span>
    <?php } ?>
</div>
